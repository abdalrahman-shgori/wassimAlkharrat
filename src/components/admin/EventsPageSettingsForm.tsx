...
export default function EventsPageSettingsForm() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (!response.ok || !data.success) {
        setError(data.error || "Failed to load events page settings");
        return;
      }
      ...
      console.error(err);
      setError("Failed to load events page settings");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/events-page-settings/upload", {
        method: "POST",
        headers: baseHeaders,
        body: formData,
      });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        setError(data.error || "Failed to save events page settings");
        return;
      }

      setSuccess("Events page hero image saved");
    } catch (err) {
      console.error(err);
      setError("Failed to save events page settings");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return <div className={styles.loading}>Loading events page settings...</div>;
  }
    ...
    <form className={styles.form} onSubmit={handleSave}>
      ...
      <div className={styles.formGroup}>
        ...
        <p style={{ margin: "0 0 0.75rem 0", color: "#666" }}>
          Upload a high-quality background image for the events page hero section.
        </p>
        <ImageUpload
          imageUrl={heroImage}
